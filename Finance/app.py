...
db = SQL("postgresql://postgres:JvCWDFJmtwOLxdIkCdLoDaiwjUgsbkvU@roundhouse.proxy.rlwy.net:26564/railway")
# railway: anastasiya24@imbetain.com; username: anastasiya
...
def buy():
    ...
    if request.method == "POST":
        ...
        # Now update if already exists otherwise insert a new row into Transactions table
        date = datetime.datetime.now()
        already_exists = db.execute("SELECT * FROM transactions where userid = ? and symbol = ? and price = ?",userid,symbol,str(d_s["price"]))
        
        if(len(already_exists)>0):
            db.execute("UPDATE transactions SET shares = ? where userid = ? and symbol = ? and price = ?", already_exists[0]["shares"]+shares,userid,symbol,str(d_s["price"]))
        else:
            db.execute("INSERT INTO transactions (userid,symbol,price,shares) VALUES (?,?,?,?)",
                      userid, symbol, d_s["price"], shares)
...
def quote():
    ...
    if not d:
        ...
        return render_template("quoted.html", price=usd(d["price"]), symbol=d["symbol"])
...
def register():
    ...
    if request.method == "POST":
        ...
        #insert user successully
        db.execute("INSERT INTO users (username, hash) VALUES(?, ?)", request.form.get("username"),
                   generate_password_hash(request.form.get("password")))

        # fetch the row from table which we have just inserted to get the userid and add it to the session (make the user login)
...
def sell():
    ...
    if request.method == "GET":
        ...
        symbol = request.form.get("symbol")
        shares = int(request.form.get("shares"))
        curr_price1 = lookup(symbol)
        curr_price = curr_price1["price"]
        userid = session["user_id"]
        trans_d = db.execute("SELECT * from transactions where userid =?", userid)
        user_d = db.execute("SELECT * from users where id = ?", userid)
        ...
        for trans in trans_d:
            if trans["symbol"] == symbol:
                if trans["shares"] >= shares:
                    ...
                    purchase_price = trans["price"]
                    error = 0
        if error == 1:
            return apology("TOO MANY SHARES ")

        # updating user's cash
        updated_cash = user_d[0]["cash"]+(shares*curr_price)
        db.execute("UPDATE users SET cash = ? where id = ?", updated_cash, userid)

        # updating no. of shares
        if updated_shares == 0:
            db.execute("DELETE FROM transactions where userid = ? and symbol = ? and price = ?",userid, symbol, str(purchase_price))
        else:
            db.execute("UPDATE transactions SET shares = ? where userid = ? and symbol = ? and price = ?", updated_shares, userid, symbol, str(purchase_price))

        # Now inserting a new row into history table
        date = datetime.datetime.now()
        db.execute("INSERT INTO history (userid,symbol,shares,price,transacted) VALUES (?,?,?,?,?)",
                   userid, symbol, -shares, curr_price, date)
